  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <h2 style={{ margin: 0, fontSize: 22, fontFamily: "'Fraunces', serif", color: P.text }}>Add Pages</h2>
        <div style={{ width: 80 }} />
      </div>
      {pages.length > 0 && (
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {pages.map((p, i) => (
            <div key={i} style={{ background: P.card, borderRadius: 12, padding: "14px 18px", boxShadow: P.shadow, display: "flex", alignItems: "flex-start", gap: 12 }}>
              <span style={{ background: P.accentLight, color: P.accent, borderRadius: 8, padding: "4px 10px", fontSize: 13, fontWeight: 700, flexShrink: 0, marginTop: 2 }}>{i + 1}</span>
      ...
      {pages.length > 0 && (
        <button style={{ ...secondaryBtn, width: "100%", background: P.successLight, color: P.success, borderColor: P.success }} onClick={onFinish}>
          Finish Book ({pages.length} page{pages.length !== 1 ? "s" : ""}) →
        </button>
      )}
    </div>
  );
